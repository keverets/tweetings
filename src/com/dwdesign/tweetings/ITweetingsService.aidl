...
interface ITweetingsService {
	...
	int getHomeTimelineWithSinceId(in long[] account_ids, in long[] max_ids, in long[] since_ids);
	int getMentionsWithSinceId(in long[] account_ids, in long[] max_ids, in long[] since_ids);
	int getReceivedDirectMessagesWithSinceId(in long[] account_ids, in long[] max_ids, in long[] since_ids);
	int getSentDirectMessagesWithSinceId(in long[] account_ids, in long[] max_ids, in long[] since_ids);
	int refreshAll();
	...
	int updateBannerImage(long account_id, in Uri image_uri, boolean delete_image);
	...
	int insertStreamToInbox(long account_id, in Intent intent);
}
